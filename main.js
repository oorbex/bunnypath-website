...
function renderDaily(){const el=document.querySelector('#daily-cards');el.innerHTML=dailySets[dailyRound%dailySets.length].map(n=>{const a=normalizeActivity(n);return `<button class="activity-mini" aria-haspopup="dialog" data-detail="${esc(n)}"><span class="emoji" aria-hidden="true">${a.emoji}</span><span><b>${esc(n)}</b><small>${esc(a.badges.slice(0,2).join(' · '))}</small></span><span class="mini-arrow" aria-hidden="true">${popupIcon}</span></button>`}).join('');animateRefresh(el)}
renderDaily();
...
function renderAge(){const age=Number(slider.value);ageWorld.setAge(age);const label=age===0?'Under 1 year':age===1?'1 year old':`${age} years old`;document.querySelector('#age-readout').textContent=label;slider.setAttribute('aria-valuetext',label);slider.style.background=`linear-gradient(to right,var(--coral) ${age/12*100}%,#d9dfc9 ${age/12*100}%)`;document.querySelector('#age-stage').textContent=ageStage(age);document.querySelector('#age-badge').textContent=age===0?'UNDER 1':`AGE ${age}`;const el=document.querySelector('#age-cards');el.innerHTML=agePreview(age,ageOffset).map(a=>`<button class="age-card" data-kind="${esc(a.type)}" aria-haspopup="dialog" data-detail="${esc(a.title)}"><span class="emoji" aria-hidden="true">${a.emoji}</span><span><small>${ageLabels[a.type]}</small><h3>${esc(a.title)}</h3><p>${esc(a.meta)}</p></span><span aria-hidden="true">${popupIcon}</span></button>`).join('');animateRefresh(el)}
...
document.querySelector('#universe-fallback').innerHTML=library.slice(0,12).map(a=>`<button aria-haspopup="dialog" data-detail="${esc(a.name)}"><span aria-hidden="true">${a.emoji}</span><b>${esc(a.name)}</b></button>`).join('');window.addEventListener('show-activity',e=>showDetail(e.detail));
let filterLimit=6;const filterState=new Set();const search=document.querySelector('#activity-search');
function renderFilters(){const list=filterActivities(search.value,[...filterState]);document.querySelector('#filter-more').hidden=list.length<=filterLimit;document.querySelector('#filter-count').textContent=`Showing ${Math.min(filterLimit,list.length)} of ${list.length} examples`;document.querySelector('#filter-grid').innerHTML=list.length?list.slice(0,filterLimit).map(a=>`<button class="filter-card" aria-haspopup="dialog" data-detail="${esc(a.name)}"><span class="emoji" aria-hidden="true">${a.emoji}</span><span class="card-arrow" aria-hidden="true">${popupIcon}</span><h3>${esc(a.name)}</h3><p>${esc(a.age)} · ${esc(a.time)}</p></button>`).join(''):'<div class="empty-results"><b>A little more room to play?</b><p>Try a different search or fewer filters.</p><button class="text-button" data-reset>Clear filters and search ↺</button></div>'}
...
const frameworkCard=(name,label,index)=>`<button class="framework-card" aria-haspopup="dialog" data-framework="${esc(name)}"><span class="framework-card-top"><span class="framework-index" aria-hidden="true">${String(index+1).padStart(2,'0')}</span><span class="framework-open" aria-hidden="true">${popupIcon}</span></span><b>${esc(name)}</b>${label?`<span class="framework-card-label">${esc(label)}</span>`:''}<span class="framework-card-action">See how it shapes play</span></button>`;
